 <template>
  <div class="page">
    <top-header header-title="兑换红包" go-back='true'></top-header>
...
export default {
  ...
  computed: {
    ...
    status() {
      const status = (/^\d+$/gi.test(this.exchangeCode)) && (/^\w{4}$/gi.test(this.codeNumber));
      return status;
    },
  },
  methods: {
    ...
    async exchange() {
      if (this.status) {
        const res = await exChangeHongbao(this.userInfo.user_id,
                                          this.exchangeCode, this.codeNumber);
        // 不成功则弹出提示框
        if (res.data.message) {
          this.getCaptchaCode();
          this.showAlert = true;
