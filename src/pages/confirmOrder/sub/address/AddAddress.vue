 <template>
  <div class="address_page">
    <top-header header-title="添加地址" go-back='true'></top-header>
...
export default {
  ...
  methods: {
    ...
    async addAddress() {
      ...
      const res = await postAddAddress(this.userInfo.user_id, this.searchAddress.name,
                                    this.address_detail, this.geohash,
                                    this.name, this.phone, this.anntherPhoneNumber,
                                    0, this.sex, this.tag, this.tag_type);
      // 保存成功返沪上一页，否则弹出提示框
      if (res.data.message) {
        this.showAlert = true;
