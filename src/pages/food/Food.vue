<template>
  <div class="food_container">
    <top-header :header-title="headTitle" goBack="true"></top-header>
...
export default {
  ...
  methods: {
    ...
    async initData() {
      ...
      // 获取筛选列表的配送方式
      res = await foodDelivery(this.latitude, this.longitude);
      this.Delivery = res.data;
      // 获取筛选列表的商铺活动
      res = await foodActivity(this.latitude, this.longitude);
...
.sort_container {
  ...
  .sort_item {
    @mixin fsc 0.55rem, #444;
    @mixin wh 33.3%, 1.6rem;
    text-align: center;
    line-height: 1rem;
    .sort_item_container {
      @mixin wh 100%, 100%;
