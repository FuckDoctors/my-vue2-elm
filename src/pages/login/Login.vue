<template>
  <div class="loginContainer">
    <top-header :header-title="loginWay? '登录':'密码登录'" goBack="true">
  ...
  methods: {
    ...
    async getVerifyCode() {
      if (this.rightPhoneNumber) {
        ...
        if (exsis.data.message) {
          this.showAlert = true;
          this.alertText = exsis.data.message;
          return;
        } else if (!exsis.data.is_exists) {
          this.showAlert = true;
    ...
    async mobileLogin() {
      if (this.loginWay) {
        ...
        // 手机号登录
        const res = await sendLogin(this.mobileCode, this.phoneNumber, this.validate_token);
