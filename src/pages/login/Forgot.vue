<template>
  <div class="restContainer">
    <top-header header-title="重置密码" goBack="true"></top-header>
    <form class="restForm">
  ...
  methods: {
    ...
    async getVerifyCode() {
      if (this.rightPhoneNumber) {
        ...
        if (getCode.data.message) {
          this.showAlert = true;
          this.alertText = getCode.data.message;
          return;
        }
    ...
    async resetButton() {
      ...
      // 发送重置信息
      const res = await changePassword(this.phoneNumber, this.oldPassWord, this.newPassWord,
                                      this.confirmPassWord, this.mobileCode);
      if (res.data.message) {
        this.showAlert = true;
        this.alertText = res.data.message;
        this.getCaptchaCode();
...
.restForm {
  ...
  .phone_number {
    /* padding: .3rem .8rem; */ /* 这个的padding跟input_container的padding不一致，导致账号输入框比较矮 */
  }
