                ...
                <li v-for="(item,index) in menuList" :key="index">
                  ...
                  <section v-for="(foods,foodindex) in item.foods" :key="foodindex" class="menu_detail_list">
                    ...
                    <footer class="menu_detail_footer">
                      ...
                      <shopping-cart :shopId='shopId' :foods='foods' @moveInCart="listenInCart" @showChooseList="showChooseList" @showReduceTip="showReduceTip" @showMoveDot="showMoveDotFun"></shopping-cart>
                    </footer>
                  </section>
                </li>
              </ul>
                ...
                <ul>
                  <li v-for="(item, index) in cartFoodList" :key="index" class="cart_food_li">
                    ...
                    <section class="cart_list_control">
                      <span @click="removeOutCart(item.categoryId, item.itemId, item.foodId, item.name, item.price, item.specs)">
                        <svg>
                          <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#cart-minus"></use>
                        </svg>
                      </span>
                      <span class="cart_num">{{item.num}}</span>
                      <svg class="cart_add" @click="addToCart(item.categoryId, item.itemId, item.foodId, item.name, item.price, item.specs)">
      ...
      <transition name="fadeBounce">
        <div class="specs_list" v-if="showSpecs">
          ...
          <footer class="specs_footer">
            ...
            <div class="specs_addto_cart" @click="addSpecs(choosedFoods.categoryId, choosedFoods.itemId, choosedFoods.specfoods[specsIndex].foodId, choosedFoods.specfoods[specsIndex].name, choosedFoods.specfoods[specsIndex].price, choosedFoods.specifications[0].values[specsIndex], choosedFoods.specfoods[specsIndex].packingFee, choosedFoods.specfoods[specsIndex].skuId, choosedFoods.specfoods[specsIndex].stock)">加入购物车</div>
          </footer>
        </div>
      </transition>
    </section>
...
export default {
  ...
  methods: {
    ...
    async initData() {
      ...
      // 数据转换
      // 排查半天才理清。。。开始显示的不对，点击一个商品的添加，所有的都添加了一遍。
      // 因为categoryId，itemId，foodId等都为undefined
      const tmp = res.data;
      tmp.forEach((item) => {
        Object.values(item.foods).forEach((food) => {
          food.categoryId = food.category_id;
          food.itemId = food.item_id;
          Object.values(food.specfoods).forEach((spec) => {
            spec.foodId = spec.food_id;
            spec.skuId = spec.sku_id;
            spec.packingFee = spec.packing_fee;
            spec.itemId = spec.item_id;
          });
        });
      });
      this.menuList = tmp;
      ...
      // 评论Tag列表
      res = await ratingTags(this.shopId);
            ...
            Object.keys(this.shopCart[item.foods[0].category_id][itemid]).forEach((foodid) => {
              ...
              if (item.type === 1) {
                ...
                if (foodItem.num > 0) {
                  ...
                  this.cartFoodList[cartFoodNum].categoryId = item.foods[0].category_id;
                  this.cartFoodList[cartFoodNum].itemId = itemid;
    ...
    async changeTgeIndex(index, name) {
      ...
      const res = await getRatingList(this.shopId, this.ratingOffset, name);
      this.ratingList = [...res.data];
      this.$nextTick(() => {
  ...
  .menu_right {
    ...
    .menu_detail_header {
      ...
      .menu_detail_header_right {
        @mixin wh .5rem, 1rem;
        display: block;
        @mixin bgImg '../../assets/images/icon_point.png';
